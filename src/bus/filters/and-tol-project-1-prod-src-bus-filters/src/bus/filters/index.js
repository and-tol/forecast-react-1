...
export const Filters = () => {
  ...
  const btnJSX =
    ...
    (isFilter && !isReset && <button onClick={handlerFilter}>Отфильтровать</button>) ||
    (isFilter && isReset && <button onClick={handlerReset}>Сбросить</button>);

  return (
    <div className={Styles['filter']}>
      <span className={Styles['checkbox']} data-type='cloudy' ref={inputCloudy} onClick={handleSetCloudy}>
        Облачно
      </span>
      <span className={Styles['checkbox']} data-type='sunny' ref={inputSunny} onClick={handleSetSunny}>
        Солнечно
      </span>
